...
class Plugin {
  ...
  massMove() {
    ...
    for (let planet of asteroids) {
      ...
      {
        // ui.terminal.current.print("..................");
        // ui.terminal.current.print("moves"+moves);
        // ui.terminal.current.print("..................");
        return [moves, asteroids.length];
      }
      
      // ui.terminal.current.print("..................");
      // ui.terminal.current.print(Object.keys(asteroids).length);
      // ui.terminal.current.print("..................");
      // ui.terminal.current.print(planet.locationId);
...
function distributeSilver(fromId, maxDistributeEnergyPercent) {
 ...
 debugger;
  let candidates_Ori;
  try {
    candidates_Ori = df.getPlanetsInRange(fromId, maxDistributeEnergyPercent);
  } catch (error) {
    return false;
  }

  const candidates_ = candidates_Ori
    .filter(p => {
      if (!players.includes(p.owner) || p.planetType !== PlanetType.TRADING_POST) {
        return false;
      }

      // Lvl 1-3 can only send to their level
      if (from.planetLevel <= 3) {
        return p.planetLevel === from.planetLevel
      }

      // Allow lvl 4 to send to lvl 3, etc
